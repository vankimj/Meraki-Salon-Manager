...
- [x] **Heal path validated under fault injection** (2026-05-11). `scripts/test-heal-usersFull.cjs inject` deleted `tenants/meraki/data/usersFull`; hard-refresh of Admin → Users fired `[healUsersFullIfMissing] rebuilt data/usersFull from staffEmails (14 users)` in console; Users tab fully repopulated; `verify` confirmed all 14 users rebuilt with correct admin/tech roles and `_healed: true` forensic markers. Original backup restored after test. Re-run any time via `node scripts/test-heal-usersFull.cjs inject|verify`.
