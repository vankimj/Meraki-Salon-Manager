...
export default function Slide({ slide, isDefault }) {
  ...
  return (
    <div style={{ minWidth: '100%', height: '100%', display: 'flex', padding: 10, gap: 9 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', borderRadius: 10, border: '1px solid var(--tm-border, #e0e0e0)', overflow: 'hidden', background: 'var(--tm-bg, #efefef)' }}>
        ...
        <div style={{ flexShrink: 0, height: 54, background: '#fff', borderTop: '3px solid var(--tm-primary, #2D7A5F)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 14px' }}>
          {hasName
            ? <span style={{ fontFamily: 'Georgia,Palatino,serif', fontSize: 15, letterSpacing: '.025em', color: 'var(--tm-dark, #1a1a1a)', textAlign: 'center', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}>{slide.name}</span>
      ...
      {/* Right: QR panel */}
      <div style={{ flex: 1, background: '#fff', borderRadius: 10, border: '1px solid var(--tm-border, #e0e0e0)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '10px 10px 28px', position: 'relative', overflow: 'hidden', minHeight: 0 }}>
        <QRPanel slide={slide} />
      </div>
    </div>
  );
